...
class AssessmentService {
  ...
  // ── Assessments ────────────────────────────────────────
  // Filter/query construction lives in repositories/assessmentRepository.js
  // (buildWhere, findPage, findRankFiltered, queryLocationReport) — the one
  // genuinely complex piece of assessment data access. Everything below
  // that creates/updates/transitions an assessment stays here: those are
  // single transactional writes mixed with business rules, not query
  // construction.

  async listAssessments({ page = 1, pageSize = 20, rank, ...filters } = {}) {
    const where = assessmentRepository.buildWhere(filters);

    const pageNum = Math.max(1, Number(page) || 1);
    const pageSizeNum = Math.min(100, Math.max(1, Number(pageSize) || 20));

    if (rank) {
      const matching = await assessmentRepository.findRankFiltered(where, Number(rank));
    ...
    const { data, total } = await assessmentRepository.findPage(where, {
  ...
  async exportAssessments({ rank, ...filters } = {}) {
    ...
    if (rank) {
      const assessments = await assessmentRepository.findRankFiltered(where, Number(rank), {
        select: assessmentRepository.EXPORT_SELECT,
        cap: EXPORT_ROW_CAP,
      });
      return { assessments };
    }

    const assessments = await assessmentRepository.findExportRows(where, EXPORT_ROW_CAP);
    return { assessments };
  }
   ...
   * Query itself lives in assessmentRepository.queryLocationReport; this
   * just maps the raw rows to the API's camelCase shape.
   */
  async getLocationReport(filters = {}) {
    const rows = await assessmentRepository.queryLocationReport(filters);
